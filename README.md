...
FocusDiT studies how the feed-forward network (FFN) in a diffusion transformer contributes to detailed text-to-image synthesis. It treats FFN hidden units as a vocabulary of visual concepts and focuses FFN updates on image tokens that need finer detail.
...
The full FocusDiT method includes cross-block shared FFN experts. The released model code assigns different FFN hidden widths to individual blocks and does not implement cross-block sharing. Q-Mask can also support optional mask-based inference acceleration; the example script does not implement that option.

## Training and evaluation

Training scripts and dataset preparation code are not included in this release. The FocusDiT training setup is described in Section 5 of the [manuscript](https://arxiv.org/abs/2606.02090).
